...
import it.itdistribuzione.gilda.gen.controllerBaseLogic.sicurezza.MenuUtenteAbstractPage;
 ...
 */
public class MenuUtentePage extends MenuUtenteAbstractPage {
  ...
  public void execLoad() throws Exception {
    getForm().getAnagrafica().setDataSource(SecurityDao.getMenuUtente());
  }

  @Override
  public boolean execCommit() throws Exception {
    if (super.execCommit()) {
      SecurityDao.normalizzaMenuUtenteRuoli();
      return true;
    } else {
      return false;
    }
  ...
  public void normalizzaPressed() throws Exception {
    ...
    int i = 0;
    for (Sec_dec_menuutenteRowBean rowBean : getForm().getAnagrafica().getDataSource()) {
  ...
  public void clonaPressed() throws Exception {
    if (getForm().getAnagrafica().size() > 0) {
      Sec_dec_menuutenteRowBean funzioniRowBean = getRowBean();

      onInsert();
      getRowBean().copyFromDataSource(funzioniRowBean);
      getRowBean().setCodmenuutente(getRowBean().getCodmenuutente() + "-COPY");
